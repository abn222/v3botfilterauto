...
async def auto_filter(bot, update):
    ...
    if filters:
        #results.append(
        #        [
        #            InlineKeyboardButton("⭕️ JOIN OUR MAIN CHANNEL ⭕️", url="https://t.me/parkboyschats")
        #        ]
        #    ) 
        ...
        Send_message=await bot.send_video(
                ...
                caption=f"Couldn't Find This Movie.Please Try Again Or Search On Our <b><a href='https://t.me/parkboyschat'>Channel</a></b>. \n\nഈ സിനിമയുടെ ഒറിജിനൽ പേര് ഗൂഗിളിൽ പോയി കണ്ടെത്തി അതുപോലെ ഇവിടെ കൊടുക്കുക 🥺",
                parse_mode="html",
    ...
    if len(results) == 0: # double check
        ...
        try:
            await bot.send_message(
                chat_id = update.chat.id,
                text=f"🤖 @parkboyschat 🤖 🄿🄱 <code><b><i>{(len_results)}</i></b></code> Your result From 🄿🄱: <code><b><i>{query}</i></b></code>, Requested By <b><code>{update.from_user.first_name}</code></b>",
